...
class ExamController extends Controller {
    ...
    public function removeExam(Request $request) {
        ...
        $result = Result::where('quiz_id', $quizId)->where('user_id', $userId)
                    ->exists();
        ...
        if ($result) {
            ...
            return redirect()->back()->with('message', 'Exam is now assigned to that user!');
        }
    } 
    ...
    public function getQuizQuestions(Request $request, $quizId) {
        ...
        $quiz = DB::table('quiz_user')->where('user_id', $authUser)
            ->pluck('quiz_id')->toArray();

        if(!in_array($quizId, $quiz)) {
        ...
        $authUserHasPlayedQuiz = Result::where(['user_id'=>$authUser, 
            'quiz_id'=>$quizId])->get();
        ...
        if(!in_array($quizId, $wasCompleted)) {
            return redirect()->to('/home')->with('error', 'You already 
                participated in this exam');
        }
